...
import { avialiableWebSocketEvents, currentSocketsEventInfo } from 'src/auth/constants/socket.event';
...
export class SocketTasks {
  ...
  private async _sendMessage(): Promise<void> {
    const socketEventInfo:string =
      await this._cacheManager.get(currentSocketsEventInfo);
    ...
    for (const socketEvent of  JSON.parse(socketEventInfo)){
      ...
      switch(socketEvent.event){
        ...
        case avialiableWebSocketEvents.userBookmarksList :{
          dataToSend = await this._getUserBookmarksList(socketEvent.data.userId)
          break;
        }
        default: return
      }
...
private async _getUserBookmarksList(userId:string):Promise<any>{
  ...
  const userBookmarks =
  await this._bookmarkRepository.getUserBookmarksList(userId, true);
